{{-- resources/views/admin/our-services/_form.blade.php --}}
<style>
:root {
    --bg:#f1f2f4; --surface:#ffffff; --border:#e3e5e8; --text-primary:#202223; --text-secondary:#6d7175;
    --text-hint:#8c9196; --accent:#303d89; --accent-light:#f0f1fc; --red:#b22222;
...
<div class="app-content content container-fluid">
    <div class="os-form-page">
        <div class="page-header">
            <div>
                <h1>{{ isset($service) ? 'Edit Our Product' : 'Add Our Product' }}</h1>
                <div class="crumb">
                    <a href="{{ route('admin.dashboard') }}">Dashboard</a><span>›</span>
                    <a href="{{ route('admin.our-services.index') }}">Our Product</a><span>›</span>
        ...
        <form method="POST" action="{{ isset($service) ? route('admin.our-services.update', $service->id) : route('admin.our-services.store') }}" class="save-form">
            @csrf
            ...
            <div class="section-card">
                ...
                <div class="section-card-body">
                    <div class="field-group">
                        ...
                        <input type="text" name="title" class="field-input" value="{{ old('title', $service->title ?? '') }}" placeholder="e.g. Handmade Rugs">
                    </div>
                    <div class="field-group">
                        <label class="field-label">Content</label>
                        <textarea name="content" id="content" class="field-textarea">{{ old('content', $service->content ?? '') }}</textarea>
                    </div>
                </div>
            </div>

            <div class="action-bar">
                <a href="{{ route('admin.our-services.index') }}" class="btn-secondary-dash">Cancel</a>
                <button type="submit" class="btn-primary-dash save-btn"><i class="fa fa-save"></i> {{ isset($service) ? 'Update' : 'Save' }}</button>
            </div>
        </form>
    </div>
</div>
