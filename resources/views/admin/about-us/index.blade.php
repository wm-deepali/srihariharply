...
<div class="main-section">
    ...
    .current-thumb { width: 108px; height: 100px; object-fit: cover; border-radius: var(--radius-sm); border: 1px solid var(--border); margin-bottom: 10px; display: block; }
    ...
    <div class="app-content content container-fluid">
        <div class="os-form-page">
            <div class="page-header">
                <div>
                    <h1>About Us Page Mgmt</h1>
                    <div class="crumb"><a href="{{ route('admin.dashboard') }}">Dashboard</a><span>›</span>About Us</div>
                </div>
            </div>

            @if (session('success'))
                <div class="section-card" style="border-color:#a9d7c4;">
                    <div class="section-card-body" style="padding:14px 20px; color:#007a5e; font-size:13px;">{{ session('success') }}</div>
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('admin.about-us.update', $aboutUs->id) }}" enctype="multipart/form-data" class="save-form">
                @csrf
                @method('PUT')

                <div class="section-card">
                    <div class="section-card-header"><h5>About Us Details</h5></div>
                    <div class="section-card-body">
                        <div class="field-group">
                            <label class="field-label">Title</label>
                            <input type="text" name="title" class="field-input" value="{{ old('title', $aboutUs->title) }}" placeholder="Title">
                        </div>
                        <div class="field-group">
                            <label class="field-label">Image</label>
                            @if($aboutUs->image)
                                <img src="{{ $aboutUs->image_url }}" class="current-thumb">
                            @endif
                        ...
                        <div class="field-group">
                            ...
                            <textarea name="content" id="content" class="field-textarea">{{ old('content', $aboutUs->content) }}</textarea>
                        </div>
                    </div>
                </div>

                <div class="action-bar">
                    <button type="submit" class="btn-primary-dash save-btn"><i class="fa fa-save"></i> Update</button>
                </div>
            </form>
        </div>
    </div>
