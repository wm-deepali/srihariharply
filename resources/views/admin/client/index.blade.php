...
<div class="main-section">
    ...
    .img-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 16px; padding: 20px; }
    .img-card { border: 1px solid var(--border); border-radius: var(--radius-sm); overflow: hidden; background: var(--surface); }
    .img-card img { width: 100%; height: 90px; object-fit: contain; background: #fafafa; display: block; padding: 8px; }
    ...
    <div class="app-content content container-fluid">
        <div class="os-page">
            ...
            <div class="os-page-header">
                <div>
                    <h1>Home Page Mgmt — Client Gallery</h1>
                    <div class="crumb"><a href="{{ route('admin.dashboard') }}">Dashboard</a><span>›</span>Client Gallery</div>
                </div>
                <a href="{{ route('admin.client.create') }}" class="btn-primary-dash"><i class="fa fa-plus"></i> Add Client Gallery</a>
            </div>
                    ...
                    <form method="GET">
                        <div class="filter-row">
                            ...
                            <div class="filter-actions">
                                <a href="{{ route('admin.client.index') }}" class="btn-filter-reset"><i class="fa fa-refresh"></i> Reset</a>
                            </div>
                        </div>
                    </form>
                </div>
                ...
                <div class="img-grid">
                    @forelse($clients as $item)
                        <div class="img-card" id="row{{ $item->id }}">
                            ...
                            @else
                                <div style="height:90px;display:flex;align-items:center;justify-content:center;background:var(--bg);color:var(--text-hint);"><i class="fa fa-image"></i></div>
                            @endif
                            <div class="img-card-body">
                                ...
                                <div style="display:flex;gap:5px;align-items:center;">
                                    ...
                                    <a href="{{ route('admin.client.edit', $item->id) }}" class="action-btn"><i class="fa fa-pencil"></i></a>
                    ...
                    @empty
                        <div class="empty-state">
                            <div class="empty-icon"><i class="fa fa-building"></i></div>
                            <strong>No client images found</strong>
                            <p>Add client logos to get started.</p>
                            <a href="{{ route('admin.client.create') }}" class="btn-primary-dash"><i class="fa fa-plus"></i> Add Client Gallery</a>
                        </div>
                    @endforelse
                </div>

                <div class="os-pagination">{{ $clients->links('pagination::bootstrap-4') }}</div>
            </div>
        </div>
    </div>
</div>
...
function deleteItem(id) {
    ...
    .then((result) => {
        if (result.isConfirmed) {
            $.ajax({
                url: "{{ url('admin/client') }}/" + id, type: 'DELETE', data: { _token: "{{ csrf_token() }}" },
