    ...
    <div class="app-content content container-fluid">
        <div class="os-page">
            ...
            <div class="os-page-header">
                <div>
                    <h1>Product Category Mgmt — Brand</h1>
                    <div class="crumb"><a href="{{ route('admin.dashboard') }}">Dashboard</a><span>›</span>Brand</div>
                </div>
                <a href="{{ route('admin.brand.create') }}" class="btn-primary-dash"><i class="fa fa-plus"></i> Add Brand</a>
            </div>
            ...
            <div class="os-card">
                    ...
                    <form method="GET">
                        <div class="filter-row">
                            ...
                            <div class="filter-group" style="flex:1">
                                <label>Search</label>
                                <input type="text" name="search" value="{{ request('search') }}" class="filter-control filter-control-wide" placeholder="Search brand…">
                            </div>
                            <div class="filter-actions">
                                <button type="submit" class="btn-filter-search"><i class="fa fa-search"></i> Search</button>
                                <a href="{{ route('admin.brand.index') }}" class="btn-filter-reset"><i class="fa fa-refresh"></i> Reset</a>
                            </div>
                        </div>
                    </form>
                </div>
                ...
                <div class="os-table-wrap">
                    ...
                    <table class="os-table">
                        <thead>
                            <tr>
                                <th><a href="{{ brandSortUrl('id') }}" class="sort-link">ID</a></th>
                                <th><a href="{{ brandSortUrl('title') }}" class="sort-link">Brand Name</a></th>
                                <th><a href="{{ brandSortUrl('status') }}" class="sort-link">Status</a></th>
                                ...
                                <tr id="row{{ $item->id }}">
                                    ...
                                    <td>
                                        <div style="display:flex;gap:6px">
                                            <a href="{{ route('admin.brand.edit', $item->id) }}" class="action-btn"><i class="fa fa-pencil"></i></a>
                                ...
                                <tr>
                                    <td colspan="4">
                                        <div class="empty-state">
                                            ...
                                            <p>Add a new brand to get started.</p>
                                            <a href="{{ route('admin.brand.create') }}" class="btn-primary-dash"><i class="fa fa-plus"></i> Add Brand</a>
                                        </div>
                                    </td>
                                </tr>
                ...
                <div class="os-pagination">{{ $brands->links('pagination::bootstrap-4') }}</div>
            </div>
        </div>
    </div>
</div>
...
function deleteItem(id) {
    ...
    .then((result) => {
        if (result.isConfirmed) {
            $.ajax({
                url: "{{ url('admin/brand') }}/" + id, type: 'DELETE', data: { _token: "{{ csrf_token() }}" },
