...
<div class="main-section">
    ...
    .thumb-placeholder { width: 48px; height: 48px; border-radius: var(--radius-sm); background: var(--bg); display: inline-flex; align-items: center; justify-content: center; color: var(--text-hint); font-size: 14px; }
    ...
    <div class="app-content content container-fluid">
        <div class="os-page">
            ...
            <div class="os-page-header">
                <div>
                    <h1>Product Category Mgmt — Category</h1>
                    <div class="crumb"><a href="{{ route('admin.dashboard') }}">Dashboard</a><span>›</span>Category</div>
                </div>
                <a href="{{ route('admin.product-category.create') }}" class="btn-primary-dash"><i class="fa fa-plus"></i> Add Category</a>
            </div>
            ...
            <div class="os-card">
                    ...
                    <form method="GET">
                        <div class="filter-row">
                            ...
                            <div class="filter-group" style="flex:1">
                                <label>Search</label>
                                <input type="text" name="search" value="{{ request('search') }}" class="filter-control filter-control-wide" placeholder="Search category…">
                            </div>
                            <div class="filter-actions">
                                <button type="submit" class="btn-filter-search"><i class="fa fa-search"></i> Search</button>
                                <a href="{{ route('admin.product-category.index') }}" class="btn-filter-reset"><i class="fa fa-refresh"></i> Reset</a>
                            </div>
                        </div>
                    </form>
                </div>
                ...
                <div class="os-table-wrap">
                    ...
                    <table class="os-table">
                        <thead>
                            <tr>
                                <th><a href="{{ pcSortUrl('id') }}" class="sort-link">ID</a></th>
                                <th>Image</th>
                                <th><a href="{{ pcSortUrl('title') }}" class="sort-link">Title</a></th>
                                <th><a href="{{ pcSortUrl('status') }}" class="sort-link">Status</a></th>
                        ...
                        <tbody>
                            @forelse($categories as $item)
                                <tr id="row{{ $item->id }}">
                                    ...
                                    <td><div class="os-title">{{ $item->title }}</div></td>
                                    <td><span class="pill {{ $item->status=='active'?'pill-active':'pill-inactive' }}">{{ ucfirst($item->status) }}</span></td>
                                    <td>
                                        <div style="display:flex;gap:6px">
                                            <a href="{{ route('admin.product-category.edit', $item->id) }}" class="action-btn"><i class="fa fa-pencil"></i></a>
                                ...
                                <tr>
                                    <td colspan="5">
                                        <div class="empty-state">
                                            <div class="empty-icon"><i class="fa fa-boxes-stacked"></i></div>
                                            <strong>No categories found</strong>
                                            <p>Add a new product category to get started.</p>
                                            <a href="{{ route('admin.product-category.create') }}" class="btn-primary-dash"><i class="fa fa-plus"></i> Add Category</a>
                                        </div>
                                    </td>
                                </tr>
                ...
                <div class="os-pagination">{{ $categories->links('pagination::bootstrap-4') }}</div>
            </div>
        </div>
    </div>
</div>
...
function deleteItem(id) {
    ...
    .then((result) => {
        if (result.isConfirmed) {
            $.ajax({
                url: "{{ url('admin/product-category') }}/" + id, type: 'DELETE', data: { _token: "{{ csrf_token() }}" },
