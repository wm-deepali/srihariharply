...
<div class="main-section">
    ...
    .os-sub { font-size: 12px; color: var(--text-hint); margin-top: 2px; }
    ...
    .thumb { width: 54px; height: 34px; object-fit: cover; border-radius: var(--radius-sm); border: 1px solid var(--border); }
    .thumb-placeholder { width: 54px; height: 34px; border-radius: var(--radius-sm); background: var(--bg); display: inline-flex; align-items: center; justify-content: center; color: var(--text-hint); font-size: 12px; }
    .pill { display: inline-flex; align-items: center; gap: 4px; font-size: 11.5px; font-weight: 600; padding: 3px 9px; border-radius: 20px; border: none; }
    ...
    <div class="app-content content container-fluid">
        <div class="os-page">
            ...
            <div class="os-page-header">
                <div>
                    <h1>Product Category Mgmt — Product</h1>
                    <div class="crumb"><a href="{{ route('admin.dashboard') }}">Dashboard</a><span>›</span>Product</div>
                </div>
                <a href="{{ route('admin.category-details.create') }}" class="btn-primary-dash"><i class="fa fa-plus"></i> Add Product</a>
            </div>
            ...
            <div class="os-card">
                    ...
                    <form method="GET">
                        <div class="filter-row">
                            <div class="filter-group">
                                ...
                                <select name="category" class="filter-control" onchange="this.form.submit()">
                            ...
                            <div class="filter-group" style="flex:1">
                                <label>Search</label>
                                <input type="text" name="search" value="{{ request('search') }}" class="filter-control filter-control-wide" placeholder="Search product…">
                            </div>
                            <div class="filter-actions">
                                <button type="submit" class="btn-filter-search"><i class="fa fa-search"></i> Search</button>
                                <a href="{{ route('admin.category-details.index') }}" class="btn-filter-reset"><i class="fa fa-refresh"></i> Reset</a>
                            </div>
                        </div>
                    </form>
                </div>
                ...
                <div class="os-table-wrap">
                    ...
                    <table class="os-table">
                        <thead>
                            <tr>
                                <th><a href="{{ pdSortUrl('id') }}" class="sort-link">ID</a></th>
                                <th>Image</th>
                                <th><a href="{{ pdSortUrl('title') }}" class="sort-link">Product</a></th>
                                <th>Category / Brand</th>
                                <th><a href="{{ pdSortUrl('status') }}" class="sort-link">Status</a></th>
                            ...
                            @forelse($products as $item)
                                <tr id="row{{ $item->id }}">
                                    ...
                                    <td>
                                        ...
                                        @if($item->content)<div class="os-sub">Code: {{ $item->content }}</div>@endif
                                    </td>
                                    <td>
                                        <div class="os-sub">{{ $item->category->title ?? '—' }}</div>
                                        <div class="os-sub">{{ $item->brand->title ?? '—' }}</div>
                                    </td>
                                    <td><span class="pill {{ $item->status=='active'?'pill-active':'pill-inactive' }}">{{ ucfirst($item->status) }}</span></td>
                                    <td>
                                        <div style="display:flex;gap:6px">
                                            <a href="{{ route('admin.category-details.edit', $item->id) }}" class="action-btn"><i class="fa fa-pencil"></i></a>
                ...
                <div class="os-pagination">{{ $products->links('pagination::bootstrap-4') }}</div>
            </div>
        </div>
    </div>
</div>
...
function deleteItem(id) {
    ...
    .then((result) => {
        if (result.isConfirmed) {
            $.ajax({
                url: "{{ url('admin/category-details') }}/" + id, type: 'DELETE', data: { _token: "{{ csrf_token() }}" },
