...
<div class="main-section">
    ...
    .os-excerpt { font-size: 12px; color: var(--text-hint); margin-top: 2px; max-width: 400px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    ...
    .thumb { width: 44px; height: 44px; object-fit: cover; border-radius: 50%; border: 1px solid var(--border); }
    .thumb-placeholder { width: 44px; height: 44px; border-radius: 50%; background: var(--bg); display: inline-flex; align-items: center; justify-content: center; color: var(--text-hint); font-size: 14px; }
    ...
    <div class="app-content content container-fluid">
        <div class="os-page">
            ...
            <div class="os-page-header">
                <div>
                    <h1>Testimonial</h1>
                    <div class="crumb"><a href="{{ route('admin.dashboard') }}">Dashboard</a><span>›</span>Testimonial</div>
                </div>
            </div>
            ...
            <div class="os-card">
                    ...
                    <form method="GET">
                        <div class="filter-row">
                            ...
                            <div class="filter-group" style="flex:1">
                                <label>Search</label>
                                <input type="text" name="search" value="{{ request('search') }}" class="filter-control filter-control-wide" placeholder="Search name…">
                            </div>
                            <div class="filter-actions">
                                <button type="submit" class="btn-filter-search"><i class="fa fa-search"></i> Search</button>
                                <a href="{{ route('admin.testimonial.index') }}" class="btn-filter-reset"><i class="fa fa-refresh"></i> Reset</a>
                            </div>
                        </div>
                    </form>
                </div>
                            ...
                            @forelse($testimonials as $item)
                                <tr id="row{{ $item->id }}">
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <div class="thumb-placeholder"><i class="fa fa-user"></i></div>
                                        @endif
                                    </td>
                                    <td><div class="os-title">{{ $item->title ?: '—' }}</div></td>
                                    <td><div class="os-excerpt">{{ Str::limit(strip_tags($item->content), 90) }}</div></td>
                                ...
                                <tr>
                                    <td colspan="6">
                                        <div class="empty-state">
                                            ...
                                            <p>Submissions from the site's contact form will show up here.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                ...
                <div class="os-pagination">{{ $testimonials->links('pagination::bootstrap-4') }}</div>
            </div>
        </div>
    </div>
</div>
...
function deleteItem(id) {
    ...
    .then((result) => {
        if (result.isConfirmed) {
            $.ajax({
                url: "{{ url('admin/testimonial') }}/" + id, type: 'DELETE', data: { _token: "{{ csrf_token() }}" },
