...
.gallery-row { display: flex; gap: 12px; align-items: flex-start; border: 1px solid var(--border); border-radius: var(--radius-sm); padding: 14px; margin-bottom: 12px; background: #fafbfc; }
.gallery-row:last-child { margin-bottom: 0; }
.gallery-row .field-group { flex: 1; margin-bottom: 0; }
.gallery-row .row-remove-wrap { padding-top: 22px; }
</style>
...
<div class="app-content content container-fluid">
    <div class="os-form-page">
        ...
        <form method="POST" action="{{ isset($image) ? route('admin.gallery-details.update', $image->id) : route('admin.gallery-details.store') }}" enctype="multipart/form-data" class="save-form">
            ...
            @isset($image)
                {{-- EDIT: single image, single gallery --}}
            ...
            @else
                {{-- CREATE: multiple rows, each with its own gallery + image --}}
                <div class="section-card">
                    ...
                    <div class="section-card-body">
                        ...
                        <div class="field-hint">Recommended size 335×285px. Har row apni gallery category + image ke liye hai — "Add More" se aur rows jodo.</div>
                    </div>
                </div>
            @endisset
...
<script>
    ...
    $(document).on('click', '#add-row-btn', function () {
        let $rows = $('#rows-wrapper .gallery-row');
    ...
    $(document).on('click', '.btn-remove-row', function () {
        if ($('#rows-wrapper .gallery-row').length > 1) {
