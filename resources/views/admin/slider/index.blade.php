...
<div class="main-section">
    ...
    .slider-page { background: var(--bg); padding: 24px 28px; min-height: 100vh; font-family: var(--font); color: var(--text-primary); }
    .slider-page * { box-sizing: border-box; }

    .slider-page-header { display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 12px; margin-bottom: 20px; }
    .slider-page-header h1 { font-size: 20px; font-weight: 650; color: var(--text-primary); margin: 0; }
    ...
    .slider-card { background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius-md); box-shadow: var(--shadow-card); overflow: hidden; }
    ...
    .slider-table-wrap { overflow-x: auto; }
    .slider-table { width: 100%; border-collapse: collapse; font-size: 13px; font-family: var(--font); }
    ...
    .slider-table tbody tr { border-bottom: 1px solid var(--border); transition: background .1s; }
    .slider-table tbody tr:last-child { border-bottom: none; }
    .slider-table tbody tr:hover { background: #fafbfc; }
    .slider-table tbody td { padding: 12px 16px; color: var(--text-primary); vertical-align: middle; }
    ...
    .slider-img { width: 96px; height: 54px; border-radius: var(--radius-sm); object-fit: cover; border: 1px solid var(--border); }
    .slider-img-placeholder { width: 96px; height: 54px; border-radius: var(--radius-sm); background: var(--bg); display: flex; align-items: center; justify-content: center; color: var(--text-hint); font-size: 16px; border: 1px solid var(--border); }

    .slider-title { font-weight: 600; color: var(--text-primary); font-size: 13px; }
    ...
    .slider-pagination { padding: 14px 20px; border-top: 1px solid var(--border); display: flex; justify-content: center; background: var(--surface); }

    @media(max-width:768px) { .slider-page { padding: 16px; } .filter-row { flex-direction: column; } .filter-control { min-width: 100%; } }
    </style>
    ...
    <div class="app-content content container-fluid">
        <div class="slider-page">
            ...
            <div class="slider-page-header">
                ...
                <div>
                    <a href="{{ route('admin.slider.create') }}" class="btn-primary-dash">
            ...
            <div class="slider-card">
                        ...
                        <div class="filter-row">
                            ...
                            <div class="filter-group" style="flex:1">
                                ...
                                <input type="text" name="search" value="{{ request('search') }}"
                                    class="filter-control filter-control-wide" placeholder="Search slider title…">
                            </div>
                            <div class="filter-actions">
                                <button type="submit" class="btn-filter-search">
                                    <i class="fa fa-search"></i> Search
                                </button>
                                <a href="{{ route('admin.slider.index') }}" class="btn-filter-reset">
                ...
                <div class="slider-table-wrap">
                    ...
                    @php
                        ...
                        function sliderSortIcon($column) {
                    ...
                    <table class="slider-table">
                        <thead>
                            <tr>
                                <th><a href="{{ sliderSortUrl('id') }}" class="sort-link">ID {!! sliderSortIcon('id') !!}</a></th>
                                <th>Image</th>
                                <th><a href="{{ sliderSortUrl('title') }}" class="sort-link">Title {!! sliderSortIcon('title') !!}</a></th>
                                <th><a href="{{ sliderSortUrl('status') }}" class="sort-link">Status {!! sliderSortIcon('status') !!}</a></th>
                        ...
                        <tbody>
                            @forelse($sliders as $item)
                                <tr id="row{{ $item->id }}">
                                    ...
                                    <td>
                                        @if($item->image)
                                            <img src="{{ $item->image_url }}" class="slider-img" alt="{{ $item->title }}">
                                        @else
                                            <div class="slider-img-placeholder"><i class="fa fa-image"></i></div>
                                        @endif
                                    </td>
                                    <td><span class="slider-title">{{ $item->title }}</span></td>
                                    <td>
                                        <button type="button"
                                    ...
                                    <td>
                                        <div style="display:flex;gap:6px">
                                            <a href="{{ route('admin.slider.edit', $item->id) }}" class="action-btn" title="Edit">
                                                <i class="fa fa-pencil"></i>
                                            </a>
                                            <button class="action-btn action-btn-danger" onclick="deleteSlider({{ $item->id }})" title="Delete">
                                ...
                                <tr>
                                    <td colspan="5">
                                        <div class="empty-state">
                                            ...
                                            <p>Add a new slider to get started.</p>
                                            <a href="{{ route('admin.slider.create') }}" class="btn-primary-dash">
                                                <i class="fa fa-plus"></i> Add Slider
                                            </a>
                                        </div>
...
function toggleStatus(id) {
    $.ajax({
        url: "{{ url('admin/slider') }}/" + id + "/toggle-status",
        type: 'POST',
        data: { _token: "{{ csrf_token() }}" },
        success: function (res) {
